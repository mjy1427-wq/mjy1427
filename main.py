...
# 카드 이미지 베이스 URL (GitHub 등에 올린 카드 이미지 경로로 수정 가능)
...
def handle_message(update, context):
    ...
    # [바카라 게임 - 크게 카드 이미지 연출 추가]
    if any(text.startswith(x) for x in ["!플 ", "!뱅 ", "!타이 "]):
        try:
            ...
            if bet_type == result:
                ...
                user['money'] += (amt * mult)
                res_t = f"✅ 축하합니다! {win_map[result]} 승리! +{amt*mult:,} G"
            else: res_t = f"❌ 아쉽습니다.. {win_map[result]} 승리. -{amt:,} G"
            
            update.message.reply_text(f"🎰 **최종 합산: {p_val} vs {b_val}**\n{res_t}")
        ...
        if now - user.get('last_mine', 0) < 40:
            ...
            return update.message.reply_text(f"⏱ **{remain}초** 후에 다시 채광할 수 있습니다.")
        ...
        user['inv'][sel] += 1
        update.message.reply_text(f"⛏ **{ORES[sel]['n']}** 획득! (내구도: {user['dur']}/{user['max_dur']})")
        ...
        update.message.reply_text("💰 등급을 선택하세요.", reply_markup=InlineKeyboardMarkup(kb))
